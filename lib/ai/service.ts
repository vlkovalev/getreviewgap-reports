...
export async function resolveShopifyDomain(productUrl: string): Promise<string> {
  try {
    ...
    if (response.ok) {
      ...
      // 1. Direct .myshopify.com reference anywhere in the HTML
      const directMatch = html.match(/([a-zA-Z0-9\-]+\.myshopify\.com)/i);
      if (directMatch?.[1]) return directMatch[1].toLowerCase();

      // 2. Shopify CDN src attributes (cdn.shopify.com/s/files/1/<shop-id>)
      //    Judge.me also accepts the shop_domain as the myshopify subdomain, so try
      //    extracting the shop name from common Shopify meta tags.
      const shopNameMeta =
        html.match(/<meta[^>]+name=["']shopify-[^"']*["'][^>]+content=["']([^"']+)["']/i) ||
        html.match(/"myshopify_domain"\s*:\s*"([^"]+)"/i) ||
        html.match(/Shopify\.shop\s*=\s*["']([^"']+\.myshopify\.com)["']/i);
      if (shopNameMeta?.[1]) return shopNameMeta[1].toLowerCase();

      // 3. Judge.me embed script carries the shop domain in its src URL
      const judgeMeScript = html.match(/cdn\.judge\.me[^"']*shop=([^&"'\s]+)/i);
      if (judgeMeScript?.[1]) {
        const decoded = decodeURIComponent(judgeMeScript[1]);
        if (decoded.endsWith(".myshopify.com")) return decoded.toLowerCase();
      }
    }
  } catch (error) {
  ...
  // Last resort: return the custom domain — the public widget will 404 but the
  // caller (fetchJudgeMePublicReviews) already tries the guessed .myshopify.com
  // prefix variant as a fallback, so this is not a dead end.
  try {
