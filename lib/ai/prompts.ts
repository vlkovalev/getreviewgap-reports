...
`.trim();
  ...
  return {
    ...
    qualityRules: [
      ...
      "BUYER LANGUAGE IS GROUPED AND RICH: Produce 20-40 verbatim phrases categorized into outcomes (what buyers felt/got), objections (what disappointed them), comparisons (what they compared it to), and unexpected_uses (uses you didn't anticipate). Return each phrase as the plain phrase only - do NOT prefix it with a category label, the category is already conveyed by which bucket (outcomes/objections/comparisons/unexpected_uses) it is placed in (e.g. outcomes: 'nice glow', objections: 'smaller than expected', comparisons: 'better than brand X', unexpected_uses: 'used for eczema'). Each phrase must appear in at least 2 reviews.",
